...
def name():
    ...
    files = client.list_objects_v2(Bucket='itexpertcertificate', Prefix = folderName)
    ...
    if not fileName in files:
        if(courseComplete):
            ...
            w, h = d.textsize(endDate, font)
            location = ((W-w)/2, 475)
            d.text(location, endDate, fill = text_color, font = font)


            imagebuffer = io.BytesIO()
            #im.save(firstName + "_" + lastName + "_" + str(studentID) + "_" + str(courseName) + ".pdf")
